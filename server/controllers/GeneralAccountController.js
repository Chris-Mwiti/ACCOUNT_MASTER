...
class GeneralAccountController {
  constructor(req, res) {
    (this.req = req), (this.res = res), (this.userId = req.userId), (this.role = req.role);
  }
  ...
  async createAccount() {
    ...
    if (!isReqValid) return this.res.status(400).json({msg: "Deposit is not enough"});

    const userModel = new UsersModel();
    const [data,error] = await TryCatchHelper(() => userModel.getUserById(this.userId));
    ...
    const creationInfo = await generalModel.createAccount().catch((err) => this.res.status(500).json({err: "Error while creating account"}));
  ...
  async getAccountsInfo(){
    ...
    const [data,error] = await TryCatchHelper(() => generalModel.getAccounts());
    if(error) return this.res.status(500).json({err: "Error while fetching accounts"});
  ...
  async getAccountInfo(){
    ...
    const generalModel = new GeneralAccount();
    const [data,error] = await TryCatchHelper(() => generalModel.getAccountInfoById(accountId));
    if(error) return this.res.status(500).json({err: "Error while getting account info"});

    if(!data) return this.res.status(400).json({err: "Account does not exist"});
